  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 0; background-color: #f6f7f8;">
      ...
      <div style="background: #FFFFFF; padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #e9ecef;">
        <h2 style="color: #1D3557; margin-top: 0; font-size: 20px; font-weight: 600;">⚠️ Gym Session Updated</h2>
        <p>Hi ${name || 'there'},</p>
      ...
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; background: white; border: 1px solid #e9ecef;">
        <tr>
          ...
          <td style="padding: 12px; border: 1px solid #e9ecef; color: #1D3557;">${sessionType ? sessionType.charAt(0).toUpperCase() + sessionType.slice(1) : 'Gym Session'}</td>
        </tr>
        ${changes.map(change => `
        <tr>
          <td style="padding: 12px; border: 1px solid #e9ecef; font-weight: bold; background: #1D3557; color: #FFFFFF;">${change.label || 'Change'}</td>
          <td style="padding: 12px; border: 1px solid #e9ecef; color: #1D3557;">${change.before} → ${change.after}</td>
        </tr>
        `).join('')}
        ${newDate ? `
        <tr>
          <td style="padding: 12px; border: 1px solid #e9ecef; font-weight: bold; background: #1D3557; color: #FFFFFF;">Updated Date</td>
          <td style="padding: 12px; border: 1px solid #e9ecef; color: #1D3557;">${formatDate(newDate)}</td>
        </tr>
        ` : ''}
        ${newTime ? `
        <tr>
        ...
        <tr>
          ...
          <td style="padding: 12px; border: 1px solid #e9ecef; color: #1D3557;">${newLocation}</td>
        </tr>
        ` : ''}
      </table>
