    ...
    <div style={{
      ...
      <main style={{
        ...
        <div style={{
          ...
          {/* Filter Buttons and View Polls Button */}
          <div style={{
            backgroundColor: '#FFFFFF',
            borderRadius: '0.75rem',
            padding: '1rem',
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                {['all', 'pending', 'accepted', 'rejected'].map((filter) => {
                  const count = filter === 'all' 
                    ? requests.length 
                    : requests.filter(r => (r.status || 'pending') === filter).length;
                                  ...
                                  <div>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                      {request.attendees.map((attendee, idx) => {
                                        const attendeeIdPath = request.individualIdsPaths && request.individualIdsPaths[idx] ? request.individualIdsPaths[idx] : null;
                                        const fileName = attendeeIdPath ? attendeeIdPath.split('/').pop() || `ID_${idx + 1}` : null;
                                        const fileUrl = attendeeIdPath ? (attendeeIdPath.startsWith('http') ? attendeeIdPath : `${API_BASE_URL}${attendeeIdPath.startsWith('/') ? attendeeIdPath : '/' + attendeeIdPath}`) : null;
                                        const attendeeName = attendee.name || attendee || `Attendee ${idx + 1}`;
