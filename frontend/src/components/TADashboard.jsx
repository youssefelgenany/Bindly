...
const TADashboard = () => {
    ...
    const loadDashboardData = async () => {
        try {
            ...
            try {
                // Use getStudentEvents which returns discover events (excludes already registered events)
                const eventsResult = await eventsApiService.getStudentEvents({});
                
                if (eventsResult.success) {
                    const eventsList = Array.isArray(eventsResult.data) 
                        ? eventsResult.data 
                        : (eventsResult.data?.events || eventsResult.data?.data?.events || []);
                    
                    console.log('🔍 TA Dashboard - Total events fetched:', eventsList.length);
                    ...
                    console.log('🔍 TA Dashboard - Upcoming count:', sortedUpcoming.length);
                    console.log('🔍 TA Dashboard - Past count:', sortedPast.length);
                    console.log('🔍 TA Dashboard - Total sorted:', sortedByDate.length);
                    console.log('🔍 TA Dashboard - Event types:', sortedByDate.map(ev => ev.type));
                    ...
                    console.log('🔍 TA Dashboard - Selected events count:', selectedEvents.length);
                    console.log('🔍 TA Dashboard - Selected event types:', selectedEvents.map(ev => ev.type));
                    ...
                    if (selectedEvents.length < 4 && sortedByDate.length > 0) {
                        console.log('🔍 TA Dashboard - Filling remaining slots. Selected:', selectedEvents.length, 'Available:', sortedByDate.length);
                        ...
                        console.log('🔍 TA Dashboard - After fill, total selected:', selectedEvents.length);
                    }
                    ...
                    if (selectedEvents.length === 0 && sortedByDate.length > 0) {
                        console.log('🔍 TA Dashboard - CRITICAL: No events selected, using emergency fallback');
                        selectedEvents.push(...sortedByDate.slice(0, 4));
                    }
                    
                    // Ensure we always have exactly 4 events for display
                    // If we have fewer than 4, pad with the first available event
                    while (selectedEvents.length < 4 && sortedByDate.length > 0) {
                        const firstEvent = sortedByDate[0];
                        // Create a unique copy to avoid duplicate key warnings
                        selectedEvents.push({ 
                            ...firstEvent, 
                            _id: `${firstEvent._id || firstEvent.id}_copy_${selectedEvents.length}`,
                            id: `${firstEvent._id || firstEvent.id}_copy_${selectedEvents.length}`
                        });
                    }
                    
                    // Always show exactly 4 events (slice to ensure)
                    const finalSelectedEvents = selectedEvents.slice(0, 4);

                    // Load ratings for selected events
                    const ratingsMap = {};
                    await Promise.all(finalSelectedEvents.map(async (ev) => {
                        if (ev._id || ev.id) {
                            try {
                    ...
                    const previewCards = finalSelectedEvents
                        .map((ev) => {
